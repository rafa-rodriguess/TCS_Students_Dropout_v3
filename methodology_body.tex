...
Because non-event follow-up is anchored to last observed VLE activity, IPCW validity in this setup relies on conditional independent censoring given $X_{it}$ rather than unconditional random censoring. A sensitivity probe exported in \texttt{table\_censoring\_anchor\_sensitivity.csv} and \texttt{table\_censoring\_informativeness\_diagnostics.csv} showed that adding engagement features increased censoring-model test AUC from $0.8938$ to $0.9299$ ($\Delta=0.0361$), and that capped-weight share in test changed from $0.6576$ (current anchor) to $0.5029$ and $0.4945$ under one- and two-week non-event trims.
...
The updated framework also includes a mechanism-validity gate and fragility diagnostics exported as \nolinkurl{table_policy_mech_validity_gate.csv}, \nolinkurl{table_policy_mech_channel_attribution.csv}, and \nolinkurl{table_policy_mech_fragility_by_scenario.csv}. These artifacts make the mechanism-aware branch auditable as an operator contract and explicitly separate robust structural contrast from channel-dependent fragility.
