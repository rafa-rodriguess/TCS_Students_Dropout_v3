...
plus the same Platt calibration protocol. The leakage guard excludes all outcome and post-outcome fields. Metrics reported are $AUC_{row,test}$, $C\text{-index}_{discrete}(T_{eval\_metrics})$, $BS_{IPCW}(T_{eval\_metrics})$, and $IBS(0{:}T_{eval\_metrics})$. This axis tests how dependent the main conclusions are on the core temporal engagement features.
