...
\begin{equation*}
\begin{aligned}
\texttt{total\_clicks}^{cf}_{it}
&=\texttt{total\_clicks}_{it}\cdot b_{it},\\
b_{it}
&=\begin{cases}
...
To avoid over-claiming from a narrow operator realization, the framework exports run-level mechanism-aware diagnostics (\nolinkurl{table_policy_activation_summary.csv}, \nolinkurl{table_policy_covariate_overwrites.csv}, \nolinkurl{table_policy_covariate_propagation_checks.csv}) alongside the RQ2 grid.
...
\State Report diagnostics: row-level $AUC_{row}$ on test person-period rows, plus $BS_{IPCW}$ and IBS at $T_{policy}$ and $T_{eval\_metrics}$.
