...
\caption{End-to-end pipeline: data integration, temporal construction, modeling, policy simulation, and fairness evaluation mapped to the seven executable tracks and the three research questions.}
...
\caption{Censoring-aware evaluation flow: the conditional censoring model determines IPCW weights and horizon stability, yielding two distinct reporting horizons.}
...
Table~\ref{tab:policy_spec} records the fixed policy parameters. The Recency trigger fires when an enrollment reaches $r^*$ consecutive weeks without LMS engagement; the active window spans $W$ weeks with exclusive upper bound.

\begin{table}[!t]
\centering
\caption{Fixed policy scenario parameters (RQ2).}
\label{tab:policy_spec}
\begin{tabular}{lr}
\hline
Parameter & Value \\
\hline
$r^*$ (recency threshold) & 1 \\
$W$ (active window length) & 2 \\
$\delta_{shock}$ (hazard reduction factor) & 0.20 \\
Mechanism-aware schedule: week-0 uplift & 0.35 \\
Mechanism-aware schedule: week-1 uplift & 0.10 \\
Decay type & step (2-week) \\
...
hazard scaled by $(1-\delta_{shock})$ within active weeks.

\item \textbf{Mechanism-aware regime ($a=1$, mech):}
covariates overwritten within active weeks (clicks boosted by schedule factor, activity/recency/streak propagated statefully), then hazard recomputed via plug-in prediction.
\end{enumerate}
...
\]
Primary reporting focuses on $\Delta S(T_{policy})$ and $\Delta S(T_{eval\_metrics})$. These outputs provide the central empirical evidence for RQ2 under the interpretive contract of simulated regime comparison.
...
\caption{Benchmark and robustness battery: the primary model is compared against alternative families (benchmark), feature subsets (ablation), unseen course runs (OOD), and an alternative endpoint (sensitivity). All axes feed a sign-consistency convergence criterion.}
...
\cite{RafaRepoTCMStudentDropout}. Horizon parameters ($T_{policy}$, $T_{eval\_metrics}$, $g_{min}$), policy specification ($r^*$, $W$, $\delta_{shock}$, schedule), and random seeds are recorded in the pipeline source. This traceability layer links the formal protocol described here to the executable notebook and exported evidence package.
...
\State Apply the Recency-triggered policy ($r^*=1$, $W=2$):
\State \hspace{6pt} \textit{Shock:} $\hat h^{(1,shock)}_{it}=\hat h^{(0)}_{it}(1-\delta_{shock})$ when active ($\delta_{shock}=0.2$).
\State \hspace{6pt} \textit{Mechanism-aware:} overwrite covariates in the active window, propagate statefully, and recompute $\hat h^{(1,mech)}_{it}=f(X^{cf}_{it})$.
\State Compute $\hat S^{(1)}_i(t)$ for each regime; report $\Delta S(T_{policy})$ and $\Delta S(T_{eval\_metrics})$.
