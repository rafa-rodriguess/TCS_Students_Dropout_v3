...
All experiments reported in this paper were executed through a scripted,
traceable pipeline centered on the project notebook
...
\texttt{outputs\_v2/tables}, and all figures/tables referenced in the paper
correspond to files produced by this export process.

To support auditing and exact replication, the pipeline records
artifact provenance during export (path and metadata in notebook outputs),
plus key protocol decisions such as split settings,
...
\texttt{table\_policy\_scenarios\_main.csv},
\texttt{table\_policy\_scenario\_params.csv},
\texttt{table\_policy\_deltaS\_by\_week\_by\_scenario.csv}, and
\texttt{table\_policy\_deltaS\_at\_horizons\_by\_scenario.csv}.

Reproducibility should be interpreted at the level of the
specified pipeline contract: same raw inputs + same protocol settings + same
export workflow produce the auditable evidence package used in this manuscript.
