...
Figure~\ref{fig:survival_baseline_rq1} connects predictive quality to decision timing. For RQ1, the baseline trajectory is the key object: periods of steeper decline define the weeks in which temporal monitoring is most decision-relevant.
...
Taken together, the discrimination and calibration evidence supports the use of the model for weekly timing signals, subject to the censoring-valid horizons established next.
...
RQ2 asks whether an explicit policy rule changes model-implied survival trajectories under fixed protocol settings. The answer is yes, but the magnitude of the contrast depends strongly on how the policy enters the system.
...
The framework answers RQ2 positively in structural terms: it compares explicit regimes on a common temporal backbone and produces interpretable survival contrasts at fixed horizons. In the present configuration, the contrast is highly scenario-dependent in the shock branch, while the mechanism-aware branch remains weak to negative under the current shared schedule.
...
The fairness interpretation pipeline is summarized in Figure~\ref{fig:fairness_pipeline_graph}. Table~\ref{tab:rq3_summary} reports the signed contrast and bootstrap uncertainty for both reported horizons.
...
The framework answers RQ3 positively at the level it proposes: it quantifies subgroup-sensitive structural contrasts and attaches uncertainty to their directional interpretation.
...
Within these limits, the section answers the paper objective directly: a reproducible temporal pipeline can translate weekly risk into explicit policy and subgroup diagnostics.